// Lo propio de LF2 Mercato en la pagina.
//
...
// La FEB solo rellena dorsal y puesto en LF Challenge: en LF2 los trae 9 de 209.
...
await arrancar({
  ...
  // Sin filtro de provincia: la LF2 es estatal y la provincia no agrupa nada.
  ...
  equipo: {
    ...
    ficha: (e) => e.urlFEB,
    // Los siete equipos que no jugaron LF2 en 25/26 no tienen ficha de esa
  ...
  // La FEB sirve el retrato por id de jugadora y en el JSON viaja la URL entera,
  // asi que se enlaza directo en vez de bajarlo.
  // La FEB sirve el retrato por URL; el de una jugadora que llega de la N1
  // andaluza lo hemos bajado nosotros y viaja como nombre de fichero.
  foto: (j) => (j.fotoLocal ? `./datos/fotos/${j.fotoLocal}` : j.foto),
  // La foto del movimiento: SOLO la que trae el propio movimiento.
  //
  // Quien llega a LF2 de fuera no esta en el censo de la categoria y no tiene
  // retrato federativo; para esos, la foto del anuncio es la unica que hay y de
  // eso ya se encarga `imagenLocal`.
  //
  // Pero hay movimientos que NO salen de un anuncio: las bajas que deduce la
  // ficha federativa. Esas no tienen imagen de post —nadie las publico— y sin
  // esto salian con el monigote gris en la cronologia mientras en su tarjeta se
  // veia su cara, porque alli la fila de plantilla si tiene el retrato del
  // censo. La misma jugadora con foto en una vista y sin ella en la otra.
